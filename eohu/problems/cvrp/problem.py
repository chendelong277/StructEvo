"""
CVRP Problem class: Integrates generator, evaluator, and prompts
"""
...
class CVRPProblem(ProblemBase):
    """CVRP problem implementation"""

    def __init__(self, seed: int = 42):
        """
        Initialize CVRP problem

        Args:
            seed: Random seed for training set generation
        """
        # Generate training set
        generator = CVRPInstanceGenerator(seed=seed)
        training_set = generator.get_training_set()

        # Initialize base class
        super().__init__(name='CVRP', training_set=training_set)

        self.evaluator = CVRPEvaluator()
        self.prompt_generator = CVRPPromptGenerator()
    ...
    def evaluate(self,
        ...
        """
        Evaluate individual on CVRP training set
    ...
    def print_training_set_info(self) -> None:
        """Print CVRP training set information"""
        for inst in self.training_set:
            n_customers = len(inst['demands']) - 1
            print(f"  {inst['name']}: {n_customers} customers, capacity={inst['capacity']}, "
                  f"total_demand={sum(inst['demands'])}")
